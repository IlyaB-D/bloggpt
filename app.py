...
def generate_content(topic: str):
    ...
    try:
        ...
        title = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Придумайте привлекательный и точный заголовок для статьи на тему '{topic}', с учётом актуальных новостей:\n{recent_news}. Заголовок должен быть интересным и передавать суть темы. НЕ используйте никакие спецсимволы кроме точек, запятых и восклицательных знаков. Запрещены символы: _ * [ ] ( ) ~ ` > # + - = | {{ }} ; : ?"
            }],
        ...
        meta_description = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Напишите мета-описание для статьи с заголовком: '{title}'. Описание должно быть полным и информативным. НЕ используйте символы кроме точек, запятых и восклицательных знаков. Запрещённые символы: _ * [ ] ( ) ~ ` > # + - = | {{ }} ; : ?"
        ...
        post_content = openai.ChatCompletion.create(
            ...
            messages=[{
                ...
                Статья должна быть информативной, логичной, иметь структуру, анализ трендов и примеры. 
                Каждый абзац не меньше 3-4 предложений. 
                Текст должен быть лёгким для чтения.
                НЕ используйте никаких символов кроме точек, запятых и восклицательных знаков.
                Запрещённые символы: _ * [ ] ( ) ~ ` > # + - = | {{ }} ; : ?
                НЕ используйте Markdown, заголовки оформляйте обычным текстом без символов решётки."""
            }],
